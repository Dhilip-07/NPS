...
class OverallReportController extends BaseController
{
    ...
    public function pdfGenerateCurrentYr($tenantId, $status, $checkyr){
        $userId = array();
        $model = new TenantModel();  
        $getTenantdata = $model->where('tenant_id', $tenantId)->first(); 
...
//  exit;
        ...
        foreach($getSurveyData as $key => $getdata){
            ...
            $surveryList = $model->where('campign_id', $getdata['campign_id'])->first();
            $model = new UserModel();
